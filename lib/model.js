...
function model (params) {
  ...
  params.init && self.on('init', params.init)
  ...
  if (params.template) {
    self.view = view({template: params.template})
    self.$el = self.view.$el
    self.$find = self.view.$find
    self.$findAll = self.view.$findAll
  }
  ...
  function mount (target) {
    ...
    self.trigger.call(self, 'mount')
  }
  function unmount () {
    self.view.unmount()
    self.trigger.call(self, 'unmount')
  }
  function update () {
    self.trigger.call(self, 'update', data, oldData)
    self.trigger.call(self, 'updated', data, oldData)
    params.template && self.trigger.call(self, 'render', data, oldData)
    params.template && self.trigger.call(self, 'rendered', data, oldData)
    oldData = {}
    for (var key in data) {
      oldData[key] = data[key]
    }
  ...
  function destroy () {
    self.trigger.call(self.view, 'destroy', {}, data)
    self.view.destroy()
    self.reset({})
    self.off()
  }
