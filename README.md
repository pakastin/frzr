...
- this.$find(query): Shortcut to this.$el.querySelector(query)
  - NEW! if you have a DOM node with a special attribute ```<div frzr="test">```, you can find this node superfast by using ```this.$find('$test')```. ```frzr```-attribute is not added to DOM.
